 ...
 * @author C2082
 */
public class ShipmentAmountR extends ShipmentAmount9 {
    ...
    public BigDecimal getARM232Value(int y, int m, Date d, int type, LinkedHashMap<String, Object> map) {
        ...
        String n_code_3C = map.get("n_code_3C") != null ? map.get("n_code_3C").toString() : "";
        StringBuilder sb = new StringBuilder();
        sb.append("SELECT ISNULL(SUM(CASE h.amtco WHEN 'P' THEN d.psamt WHEN 'M' THEN d.psamt *(-1) ELSE 0 END),0) FROM armpmm h left join cdrcus c on c.cusno =h.cusno,armacq d,cdrdta s ");
        ...
        //服务支持or后市场
        if (!"".equals(n_code_3C)) {
            sb.append("and c.SCCode ").append(n_code_3C);;
        }
    ...
    public BigDecimal getARM423Value(int y, int m, Date d, int type, LinkedHashMap<String, Object> map) {
        ...
        String n_code_3C = map.get("n_code_3C") != null ? map.get("n_code_3C").toString() : "";
        StringBuilder sb = new StringBuilder();
        sb.append("SELECT ISNULL(SUM(d.recamt),0) FROM armrec d left join cdrcus c on c.cusno =d.ivocus,armrech h where d.facno=h.facno AND d.recno=h.recno AND h.prgno='ARM423' AND h.recstat='1' AND d.raccno in('6001','6002') ");
    ...
    public BigDecimal getARM270Value(int y, int m, Date d, int type, LinkedHashMap<String, Object> map) {
        ...
        String n_code_3C = map.get("n_code_3C") != null ? map.get("n_code_3C").toString() : "";
        StringBuilder sb = new StringBuilder();
        sb.append("SELECT ISNULL(SUM(h.shpamt),0) FROM armbil h left join cdrcus c on c.cusno =h.ivocus WHERE h.rkd='RQ11' AND h.facno='${facno}'");
        if (n_code_DA != null && !"".equals(n_code_DA)) {
            sb.append(" and c.address4 ").append(n_code_DA);
        }
        if (n_code_DC != null && !"".equals(n_code_DC)) {
            sb.append(" and n_code_DC ").append(n_code_DC);
        }
        if (n_code_CD != null && !"".equals(n_code_CD)) {
            sb.append(" and c.address3 ").append(n_code_CD);
        }
        //服务支持or后市场
        if (!"".equals(n_code_3C)) {
            sb.append("and c.SCCode ").append(n_code_3C);;
    ...
    public BigDecimal getValue(int y, int m, Date d, int type, LinkedHashMap<String, Object> map) {
        ...
        StringBuilder sb = new StringBuilder();
        sb.append("select isnull(sum(case h.tax when '1' then (d.shpamts * h.ratio) else (d.shpamts * h.ratio)/(h.taxrate + 1) end),0) from cdrhad h left join cdrcus c on c.cusno =h.cusno,cdrdta d where h.facno=d.facno and h.shpno=d.shpno and h.houtsta<>'W' ");
        ...
        sb.setLength(0);
        sb.append("select isnull(sum(case h.tax when '1' then (d.bakamts * h.ratio) else (d.bakamts * h.ratio)/(h.taxrate + 1) end),0) from cdrbhad h left join cdrcus c on c.cusno =h.cusno,cdrbdta d where h.facno=d.facno and h.bakno=d.bakno and h.baksta<>'W' ");
        sb.append(" and h.cusno NOT IN ('SSD00107','SGD00088','SJS00254','SCQ00146','KZJ00029') ");
        sb.append(" and d.issevdta='Y' and h.facno='${facno}' ");
        if (!"".equals(decode)) {
            sb.append(" and h.decode ='").append(decode).append("' ");
