...
@Stateless
@LocalBean
public class BscGroupVBHSaleOrderBean implements Serializable {
    ...
    public BscGroupVBHSaleOrderBean() {
    }
    ...
    public void updataSaleOrderActualValue(int y, int m, Date d) {
        ...
        if (resultData != null) {
            erpEJB.setCompany("C");
            erpEJB.getEntityManager().createNativeQuery("delete from bsc_groupshipment where facno='VB' and year(soday)=" + y + " and month(soday) = " + m + " and type = 'SalesOrder'").executeUpdate();
    ...
    protected List<BscGroupShipment> getSalesOrderAmount(int y, int m, Date d, int type, LinkedHashMap<String, Object> map) {
        ...
        try {
            ...
            for (int i = 0; i < cdrResult.size(); i++) {
                ...
                BscGroupShipment e = new BscGroupShipment("VB", recdate, "SalesOrder", protype, protypeno, shptype);
            ...
            Logger.getLogger(BscGroupVBHSaleOrderBean.class.getName()).log(Level.SEVERE, null, ex);
        }
        return data;
    }
